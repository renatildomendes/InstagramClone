...
/**
 * Created by Renato on 17/07/2018.
 */
...
public class HomeAdapter extends ArrayAdapter<ParseObject>{
    ...
    public View getView(int position, @Nullable View convertView, @NonNull ViewGroup parent) {
        ...
        if(postagens.size() > 0){
            ImageView imageView = (ImageView) view.findViewById(R.id.imagem_lista_postagem);
            ParseObject parseObject = postagens.get(position);
            parseObject.getParseFile("imagem");
            Picasso.with(context).load(parseObject.getParseFile("imagem").getUrl()).into(imageView);
        }
